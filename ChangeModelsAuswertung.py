...
embedding_file = './res/embedding_2009_transE.vec.json'
...
pca = PCA(n_components=2)
normalized_data = pca.fit_transform(train_values_embeddings)

xs_normalized = [v[0] for v in normalized_data]
ys_normalized = [v[1] for v in normalized_data]

normalized_X = preprocessing.normalize(xs_normalized)
normalized_Y = preprocessing.normalize(ys_normalized)
...
xs_reduced_change = [xs_normalized[i] for i in change_indices]
ys_reduced_change = [ys_normalized[i] for i in change_indices]
...
evaluate_data(train_values, train_labels, test_values, test_labels, 'normalized')
